...
plot_culmen_length_figure <- function(cleaned_island_data){
    ...
    scale_fill_manual(values=c("Adelie Penguin (Pygoscelis adeliae)"= "#07004D", 
                               "Gentoo penguin (Pygoscelis papua)" = "#9E1946"))+
    ...
    labs(title ="Culmen Lengths of Male Adelie and Gentoo Penguins on Biscoe Island", 
         ...
         caption = "(Figure 1) Histogram showing distribution of culmen lengths, with species' means plotted as dotted lines.")+
    geom_vline(xintercept=mean_adelie, color="black", linetype="dashed", size=1)+
    annotate("text", x=42.4, y=14, label="P. adeliae", fontface = "italic")+
    annotate("text", x=41.9, y=-0.4, label=round(mean_adelie,3))+
    geom_vline(xintercept=mean_gentoo, color="black", linetype="dashed", size=1.25)+
    annotate("text", x=51.2, y=14, label="P. papua", fontface = "italic")+
    annotate("text", x=50.8, y=-0.4, label=round(mean_gentoo,3))+
    theme_bw()+
